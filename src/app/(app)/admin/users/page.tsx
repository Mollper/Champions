...
export default async function UsersPage({ searchParams }: PageProps<"/admin/users">) {
  ...
  return (
    <div className="container-page space-y-5 py-6 sm:py-10">
      <PageHeader eyebrow="Админ-панель" title="Пользователи" description="Смена роли применяется сразу: ментору создаётся карточка, админ получает доступ к этой панели." />
      <form className="flex flex-col gap-2 sm:flex-row">
        <input name="q" defaultValue={q} placeholder="Поиск по email или имени" className="h-11 flex-1 rounded-xl border border-line bg-surface px-3.5 text-sm outline-none focus:border-brand-400" />
        {role && <input type="hidden" name="role" value={role} />}
        <button className="h-11 rounded-xl bg-brand-600 px-5 text-sm font-semibold text-white hover:bg-brand-700">Найти</button>
      </form>
      ...
      <ul className="divide-y divide-line overflow-hidden rounded-card border border-line bg-surface">
          ...
          <li key={u.id} className="flex flex-col gap-2 p-3.5 sm:flex-row sm:items-center">
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold">
                {u.full_name || "Без имени"} {u.id === me.id && <span className="text-xs font-normal text-muted">(это вы)</span>}
              </p>
        ...
        {users.length === 0 && <li className="p-4 text-sm text-muted">Никого не нашли.</li>}
      </ul>
    </div>
  );
}
