...
export const metadata: Metadata = { title: "Чаты менторов" };
...
export default async function MentorChatsPage({ searchParams }: PageProps<"/admin/chats">) {
  ...
  return (
    <div className="container-page space-y-5 py-6 sm:py-10">
      <PageHeader eyebrow="Админ-панель" title="Чаты менторов" description="Пары ментор–ученик и их переписка (только чтение) — для разбора жалоб и контроля качества." />
              ...
              <li key={`${l.mentor_id}-${l.student_id}`}>
                ...
                >
                  <p className="truncate text-sm font-semibold">
                    {name(l.mentor)} → {name(l.student)}
                  </p>
                  <p className="text-xs text-muted">
                    {STATUS[l.status] ?? l.status} · {new Date(l.updated_at).toLocaleDateString("ru-RU")}
                  </p>
                </Link>
              </li>
            );
          })}
          {links.length === 0 && <li className="text-sm text-muted">Наставничеств пока нет.</li>}
        </ul>

        <section className="min-w-0 rounded-card border border-line bg-surface p-4">
          {!current ? (
            <p className="py-10 text-center text-sm text-muted">Выбери пару слева, чтобы посмотреть переписку.</p>
          ) : (
            <>
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line pb-3">
                <p className="text-sm font-semibold">
                  {name(current.mentor)} ↔ {name(current.student)}
                </p>
              ...
              <ul className="mt-3 max-h-[60dvh] space-y-2 overflow-y-auto">
                    ...
                    <li key={m.id} className={cn("flex", byMentor ? "justify-start" : "justify-end")}>
                      <div className={cn("max-w-[85%] rounded-2xl px-3 py-2 text-sm [overflow-wrap:anywhere]", byMentor ? "bg-brand-50" : "bg-canvas")}>
                        <p className="text-[11px] font-semibold text-muted">
                          {byMentor ? "ментор" : "ученик"} · {new Date(m.created_at).toLocaleString("ru-RU", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}
                        </p>
                ...
                {thread.length === 0 && <li className="py-6 text-center text-sm text-muted">Сообщений нет.</li>}
              </ul>
            </>
          )}
        </section>
