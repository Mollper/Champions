...
export const metadata: Metadata = { title: "Заявки менторов" };
...
export default async function ApplicationsPage() {
  ...
  return (
    <div className="container-page space-y-6 py-6 sm:py-10">
      <PageHeader eyebrow="Админ-панель" title="Заявки менторов" description="Одобренный заявитель становится ментором, и ученики видят его карточку." />
      {sorted.length === 0 && <p className="text-sm text-muted">Заявок пока нет.</p>}
            ...
            <li key={a.id} className="rounded-card border border-line bg-surface p-4 sm:p-5">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-muted">
                    {email} · {new Date(a.created_at).toLocaleDateString("ru-RU")}
                  </p>
                </div>
                <span className={cn("rounded-pill px-2.5 py-0.5 text-xs font-semibold", tone)}>{label}</span>
              </div>
              ...
              {a.contact && (
                <p className="mt-2 text-sm">
                  Контакт: <b>{a.contact}</b>
                </p>
              )}
              {a.admin_note && <p className="mt-2 text-xs text-muted">Комментарий: {a.admin_note}</p>}
