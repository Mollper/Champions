  ...
  return (
    <div className="container-page space-y-6 py-8 sm:py-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="text-sm font-semibold text-brand-600">Мини-игры UniRoute</p>
          <h1 className="font-display text-2xl font-semibold sm:text-3xl">Поступление — это тоже игра</h1>
        </div>
      </div>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {GAMES.map((g) => {
          const Icon = g.icon;
          return (
            <Link key={g.href} href={g.href} className={`group relative overflow-hidden rounded-card bg-gradient-to-br ${g.tone} p-6 text-white shadow-lift transition hover:-translate-y-1`}>
              ...
              <p className="text-sm font-semibold text-white/70">{g.subtitle}</p>
              <p className="mt-3 text-sm leading-relaxed text-white/85">{g.text}</p>
              <span className="mt-5 inline-flex items-center gap-1.5 rounded-pill bg-white/15 px-4 py-2 text-sm font-semibold backdrop-blur transition group-hover:bg-white/25">
                Играть <ArrowRight className="size-4" aria-hidden />
              </span>
            </Link>
          );
        })}
