...
export function ThemeControls({ initial, compact = false }: { initial: Theme; compact?: boolean }) {
  ...
  return (
    <div className={cn("space-y-5", compact && "space-y-4")}>
        ...
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-5 lg:grid-cols-9">
          {ACCENTS.map((a) => (
            <button
              ...
              className={cn("flex flex-col items-center gap-1.5 rounded-2xl p-2 text-xs font-medium transition", theme.accent === a.id ? "bg-canvas ring-2 ring-brand-400" : "hover:bg-canvas")}
            >
              <span className="grid size-9 place-items-center rounded-full shadow-inner ring-2 ring-white/60" style={{ background: `linear-gradient(135deg, ${a.swatch}, color-mix(in oklab, ${a.swatch} 55%, white))` }}>
          ...
          {MODES.map((m) => {
            ...
            return (
              <button
                ...
                className={cn("flex flex-col items-center gap-1.5 rounded-2xl border p-3 text-xs font-semibold transition", theme.mode === m.id ? "border-brand-400 bg-brand-50 text-brand-700" : "border-line text-ink-soft hover:border-line-strong")}
              >
        ...
        <div className="grid grid-cols-2 gap-2">
          {STYLES.map((s) => (
            <button
              ...
              className={cn("rounded-2xl border p-3 text-left transition", theme.style === s.id ? "border-brand-400 bg-brand-50" : "border-line hover:border-line-strong")}
            >
              <span
                className={cn("mb-2 block h-14 overflow-hidden rounded-xl", s.id === "glass" ? "bg-gradient-to-br from-brand-300 via-route-400/60 to-coral-400/60" : "bg-canvas")}
                aria-hidden
              >
                <span className={cn("m-2 block h-10 rounded-lg", s.id === "glass" ? "bg-white/40 ring-1 ring-white/60 backdrop-blur-md" : "bg-surface shadow-card")} />
              </span>
              <span className="block text-sm font-semibold">{s.label}</span>
              {!compact && <span className="block text-xs text-muted">{s.hint}</span>}
            </button>
          ))}
        </div>
      </Group>
      ...
      <button type="button" onClick={() => update(DEFAULT_THEME)} className="inline-flex items-center gap-1.5 text-sm font-semibold text-ink-soft hover:text-ink">
        <RotateCcw className="size-4" aria-hidden /> Вернуть как было
      </button>
    </div>
  );
}
