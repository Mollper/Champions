  ...
  return (
    <div className="mt-3 space-y-2">
      <input
        ...
        maxLength={1000}
        placeholder={t("Комментарий для заявителя (необязательно)")}
      ...
      <div className="flex gap-2">
        <button
          ...
          disabled={pending}
          onClick={() => decide(true)}
          className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl bg-success-500 text-sm font-semibold text-white hover:bg-success-700 disabled:opacity-60"
        >
          {pending ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />} {t("Одобрить")}
        </button>
        <button
          type="button"
          disabled={pending}
          onClick={() => decide(false)}
          className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl bg-surface text-sm font-semibold text-danger-700 ring-1 ring-line hover:bg-danger-50"
        >
          <X className="size-4" /> {t("Отклонить")}
        </button>
      </div>
      {error && <p className="text-sm text-danger-700">{t(error)}</p>}
    </div>
  );
}
  ...
  return (
    <span className="inline-flex items-center gap-2">
      <select
        ...
        className={cn(
          "h-9 rounded-lg border border-line bg-surface px-2 text-sm font-medium",
          value === "admin" && "text-coral-700",
          value === "mentor" && "text-brand-700",
        )}
      >
      ...
      {pending && <Loader2 className="size-4 animate-spin text-muted" aria-label={t("Сохраняем")} />}
      {error && <span className="text-xs text-danger-700">{t(error)}</span>}
    </span>
  );
}
...
export function EndMentorshipButton({ mentorId, studentId }: { mentorId: string; studentId: string }) {
  ...
  return (
    ...
    >
      {pending ? <Loader2 className="size-3.5 animate-spin" /> : <X className="size-3.5" />} {t("Завершить")}
    </button>
  );
}
