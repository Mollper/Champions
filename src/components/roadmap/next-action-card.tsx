...
export function NextActionCard({ steps, onStatus, universityName, compact = false }: Props) {
  ...
  return (
    <section
      ...
      className="relative overflow-hidden rounded-[1.5rem] bg-gradient-to-br from-coral-500 to-coral-600 p-5 text-white shadow-coral sm:p-7"
    >
        ...
        {next ? (
          ...
          >
            <div className="flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-wide text-white/85">
              <span className="rounded-pill bg-white/20 px-2.5 py-1">{t("Следующий шаг")}</span>
              <span className="inline-flex items-center gap-1 rounded-pill bg-white/15 px-2.5 py-1 normal-case tracking-normal">
                ...
                {t(CATEGORY[next.category].label)}
              </span>
              {next.status === "in_progress" && (
                <span className="rounded-pill bg-white px-2.5 py-1 normal-case tracking-normal text-coral-700">{t("в процессе")}</span>
              )}
            </div>
            ...
            {!compact && next.description && <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/85 sm:text-base">{t(next.description)}</p>}
              ...
              {next.due_date && (
                <span
                  className={cn(
                    ...
                    daysUntil(next.due_date) <= 7 && "rounded-pill bg-white px-2.5 py-0.5 text-coral-700",
                  )}
                >
                  <CalendarClock className="size-4" aria-hidden />
                  {t("до")} {formatDate(next.due_date)} · {formatDaysLeft(next.due_date)}
                </span>
              )}
              {universityName?.(next.university_id) && <span className="text-white/80">{t(universityName(next.university_id))}</span>}
            </div>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              >
                <Check className="size-4" strokeWidth={3} aria-hidden /> {t("Готово")}
              </button>
              {next.status !== "in_progress" && (
                <button
                  type="button"
                ...
                >
                  <Play className="size-4" aria-hidden /> {t("Начать")}
                </button>
              )}
              <button
                type="button"
              ...
              >
                <ChatSparkIcon className="size-4" /> {t("Спросить Юни")}
              </button>
            </div>
          </motion.div>
        ) : (
          <motion.div key="all-done" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="relative">
            ...
            <h2 id="next-action-title" className="mt-3 font-display text-2xl font-semibold">
              {t("Все шаги выполнены!")}
            </h2>
            <p className="mt-1 text-white/85">{t("Отличная работа. Если что-то изменилось — обнови анкету, и маршрут перестроится.")}</p>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="relative mt-6 border-t border-white/20 pt-4">
          <div className="flex items-center justify-between text-sm">
            <span className="text-white/85">
              {t("Пройдено")} {done} {t("из")} {total} {t("шагов")}
            </span>
            <Link href="/roadmap" className="inline-flex items-center gap-1 font-semibold hover:underline">
              {t("Весь маршрут")} <ArrowRight className="size-4" aria-hidden />
            </Link>
          </div>
