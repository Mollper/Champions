...
export function StepItem({ step, isNext, universityName, onStatus }: Props) {
  ...
  return (
    ...
    >
      {isNext && (
        <span className="absolute -top-2.5 left-4 rounded-pill bg-coral-500 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white">Следующий шаг</span>
      )}
      <div className="flex items-start gap-3 p-3.5 sm:p-4">
        <button
          ...
          role="checkbox"
          aria-checked={done}
          aria-label={done ? `Отметить «${step.title}» как невыполненный` : `Отметить «${step.title}» выполненным`}
        ...
        <div className="min-w-0 flex-1">
          <button type="button" onClick={() => setOpen((v) => !v)} aria-expanded={open} className="flex w-full items-start gap-2 text-left">
            <span className={cn("min-w-0 flex-1 font-semibold leading-snug", (done || skipped) && "text-muted line-through decoration-2")}>{step.title}</span>
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-xs">
            <span className={cn("inline-flex items-center gap-1 rounded-pill px-2 py-0.5 font-medium", cat.chip)}>
              <Icon className="size-3" aria-hidden /> {cat.label}
            </span>
            {step.due_date && (
              <span
                className={cn(
            ...
            {universityName && <span className="rounded-pill bg-canvas px-2 py-0.5 text-ink-soft">{universityName}</span>}
            {step.status === "in_progress" && <span className="rounded-pill bg-brand-50 px-2 py-0.5 font-semibold text-brand-700">в процессе</span>}
            {step.priority === 1 && !done && !skipped && <span className="font-semibold text-coral-700">важно</span>}
          </div>

          <AnimatePresence initial={false}>
            {open && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                {step.description && <p className="mt-2.5 text-sm leading-relaxed text-ink-soft">{step.description}</p>}
                <div className="mt-3 flex flex-wrap gap-2">
                  {!done && step.status !== "in_progress" && (
                    <button type="button" onClick={() => onStatus(step.id, "in_progress")} className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-brand-50 px-3 text-xs font-semibold text-brand-700 hover:bg-brand-100">
                      <Play className="size-3.5" aria-hidden /> Начать
                    </button>
                  )}
                  {!done && !skipped && (
                    <button type="button" onClick={() => onStatus(step.id, "skipped")} className="inline-flex h-8 items-center gap-1.5 rounded-lg px-3 text-xs font-semibold text-muted hover:bg-canvas">
                      <SkipForward className="size-3.5" aria-hidden /> Не актуально
                    </button>
                  )}
                  {(done || skipped || step.status === "in_progress") && (
                    <button type="button" onClick={() => onStatus(step.id, "todo")} className="inline-flex h-8 items-center gap-1.5 rounded-lg px-3 text-xs font-semibold text-muted hover:bg-canvas">
                      <CornerUpLeft className="size-3.5" aria-hidden /> Вернуть в план
                    </button>
                  )}
                </div>
              </motion.div>
