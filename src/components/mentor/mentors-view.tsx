...
export function MentorAvatar({ name, className }: { name: string; className?: string }) {
  return (
    <span className={cn("grid shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-brand-500 to-route-500 font-display font-semibold text-white", className)} aria-hidden>
      {initials(name)}
    </span>
  );
}
...
export function MentorsView({ me, mentors, current }: { me: string; mentors: MentorCard[]; current: { mentorship: Mentorship; mentor: MentorCard | null } | null }) {
  ...
  return (
    <div className="space-y-6">
      {current && (
        <section className={cn("rounded-card border p-4 sm:p-5", current.mentorship.status === "active" ? "border-route-100 bg-route-50/70" : "border-brand-100 bg-brand-50/60")}>
          <div className="flex flex-wrap items-center gap-3">
            <MentorAvatar name={current.mentor?.display_name ?? "Ментор"} className="size-12 text-base" />
            <div className="min-w-0 flex-1">
              <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-muted">
                {current.mentorship.status === "active" ? (
                  <>
                    <CheckCircle2 className="size-3.5 text-route-600" aria-hidden /> Твой ментор
                  </>
                ) : (
                  <>
                    <Clock3 className="size-3.5 text-brand-600" aria-hidden /> Заявка ждёт ответа
                  </>
                )}
              </p>
              <p className="truncate font-semibold">{current.mentor?.display_name ?? "Ментор"}</p>
              <p className="text-sm text-ink-soft">{current.mentor?.headline}</p>
            </div>
            <div className="flex w-full gap-2 sm:w-auto">
              {current.mentorship.status === "active" && (
                <Link href="/messages" className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl bg-brand-600 px-4 text-sm font-semibold text-white hover:bg-brand-700 sm:flex-none">
                  <MessagesSquare className="size-4" aria-hidden /> Чат
                </Link>
              )}
              <button
                type="button"
              ...
              >
                <X className="size-4" aria-hidden /> {current.mentorship.status === "active" ? "Завершить" : "Отозвать"}
              </button>
            </div>
          </div>
        </section>
      ...
      {others.length === 0 ? (
        <div className="rounded-card border border-dashed border-line-strong bg-surface p-8 text-center">
          ...
          <p className="mt-3 font-semibold">Менторов пока нет</p>
          <p className="mx-auto mt-1 max-w-sm text-sm text-muted">Скоро здесь появятся проверенные наставники. Ты сам поступил и хочешь помогать? Подай заявку.</p>
        </div>
      ) : (
            ...
            return (
              <li key={m.user_id} className="flex flex-col rounded-card border border-line bg-surface p-4 sm:p-5">
                <div className="flex items-start gap-3">
                  ...
                  {!m.accepting && <span className="shrink-0 rounded-pill bg-canvas px-2 py-0.5 text-[11px] font-semibold text-muted">не набирает</span>}
                </div>
                  ...
                  {openFor === m.user_id ? (
                    <div className="space-y-2">
                      <textarea
                        ...
                        maxLength={500}
                        rows={3}
                        placeholder="Пара слов о себе: класс, цель, с чем нужна помощь"
                    ...
                    <Button size="sm" variant={isCurrent ? "soft" : "primary"} className="w-full" disabled={Boolean(current) || !m.accepting} onClick={() => setOpenFor(m.user_id)}>
                      {isCurrent ? "Уже выбран" : current ? "Сначала заверши текущую заявку" : "Выбрать ментором"}
                    </Button>
                  )}
                </div>
              </li>
