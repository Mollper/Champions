    ...
    <div className="space-y-6">
      ...
      <section>
        <h2 className="font-semibold">
          {t("Заявки")} <span className="text-muted">· {requests.length}</span>
        </h2>
        {requests.length === 0 ? (
          <p className="mt-2 text-sm text-muted">{t("Новых заявок нет — когда ученик выберет тебя, она появится здесь.")}</p>
        ) : (
          <ul className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
            {requests.map((r) => {
              const name = r.student.full_name || r.student.email || t("Ученик");
              return (
                <li key={r.student.id} className="rounded-card border border-brand-100 bg-brand-50/60 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted">{t("хочет, чтобы ты стал ментором")}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    >
                      {busy === r.student.id ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />} {t("Принять")}
                    </button>
                    <button
                      type="button"
                      disabled={busy === r.student.id}
      ...
      <section>
        <h2 className="font-semibold">
          {t("Ученики")} <span className="text-muted">· {students.length}</span>
        </h2>
        {students.length === 0 ? (
          <p className="mt-2 text-sm text-muted">{t("Пока нет активных учеников.")}</p>
        ) : (
                ...
                <li key={r.student.id} className="flex flex-col rounded-card border border-line bg-surface p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-xs text-muted">
                        {t(r.profile?.completed_at ? `${GRADE_LABEL[r.profile.grade ?? 0] ?? "—"} · анкета заполнена` : "анкета не заполнена")}
                      </p>
                    </div>
                  ...
                  {r.profile && (
                    <p className="mt-3 line-clamp-2 text-sm text-ink-soft">
                      {t(r.profile.interests.map((f) => FIELD_LABEL[f] ?? f).join(", ") || "Интересы не указаны")}
                      {t(r.profile.target_countries.length ? ` · ${r.profile.target_countries.map((c) => COUNTRY_NAME[c] ?? c).join(", ")}` : "")}
                    </p>
                  )}
                  <div className="mt-3">
                  ...
                  <div className="mt-4 flex gap-2">
                    <Link
                      ...
                      className="inline-flex h-10 flex-1 items-center justify-center gap-1 rounded-xl bg-brand-50 text-sm font-semibold text-brand-700 hover:bg-brand-100"
                    >
                      {t("Профиль")} <ChevronRight className="size-4" />
                    </Link>
                    <Link
                      href={`/messages?student=${r.student.id}`}
                      className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl bg-surface text-sm font-semibold text-ink ring-1 ring-line hover:bg-canvas"
                    >
                      <MessagesSquare className="size-4" /> {t("Чат")}
                    </Link>
                  </div>
                </li>
              );
