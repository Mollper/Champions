...
export function FavoritesView({ matches, favoriteIds, shortlistIds, recommendedIds }: { matches: MatchResult[]; favoriteIds: number[]; shortlistIds: number[]; recommendedIds: number[] }) {
  ...
  if (list.length === 0) {
    return (
      <div className="rounded-card border border-dashed border-line-strong bg-surface p-8 text-center">
        ...
        <p className="mt-3 font-semibold">Пока пусто</p>
        <p className="mx-auto mt-1 max-w-sm text-sm text-muted">
          Нажми <Heart className="inline size-4 text-coral-600" aria-label="сердечко" /> на карточке любого вуза — даже если он не прошёл по анкете, — и он появится здесь.
        </p>
        <Link href="/recommendations" className="mt-4 inline-flex h-10 items-center rounded-xl bg-brand-600 px-4 text-sm font-semibold text-white hover:bg-brand-700">
          К подборке вузов
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      {(favorites.error || shortlist.error) && <p className="rounded-xl bg-danger-50 px-3 py-2 text-sm text-danger-700">{favorites.error ?? shortlist.error}</p>}
        ...
        {list.map((m) => (
          <motion.div key={m.university.id} layout="position" exit={{ opacity: 0, scale: 0.97 }} transition={{ duration: 0.25 }}>
            <UniversityCard
              ...
              mismatch={recommendedIds.includes(m.university.id) ? null : (m.blockers[0] ?? m.concerns.find((c) => (c.weight ?? 0) > 0)?.text ?? "низкое общее совпадение с анкетой")}
            />
          </motion.div>
        ))}
      </AnimatePresence>
