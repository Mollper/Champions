 ...
 */
export async function UniversityPageView({
  university,
  matches,
  scholarships,
  complete,
  ...
  return (
    <div className="container-page space-y-5 py-5 sm:py-8">
      <Link href="/recommendations" className="inline-flex items-center gap-1.5 text-sm font-semibold text-ink-soft hover:text-ink">
        <ArrowLeft className="size-4" aria-hidden /> {t("К подборке")}
      </Link>
        ...
        <div className="space-y-4 p-4 sm:p-6">
          <div className="flex items-start gap-4">
            <div className="min-w-0 flex-1">
              <h1 className="font-display text-2xl font-semibold leading-tight tracking-tight [overflow-wrap:anywhere] sm:text-3xl">{t(u.name)}</h1>
              {u.name_ru && u.name_ru !== u.name && <p className="mt-1 text-ink-soft">{t(u.name_ru)}</p>}
              <p className="mt-1.5 flex items-center gap-1 text-sm text-muted">
                <MapPin className="size-3.5 shrink-0" aria-hidden /> {t(u.city)}, {t(COUNTRY_NAME[u.country_code] ?? u.country)}
              </p>
            </div>
            {complete && <ChanceRing chance={match.chance} tier={match.tier} size={64} />}
          </div>
          {u.description && <p className="max-w-3xl text-[15px] leading-relaxed text-ink-soft">{t(u.description)}</p>}
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start">
          ...
          {complete ? (
            <section className="rounded-card border border-line bg-surface p-5 sm:p-6">
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="font-semibold">{t("Как ты подходишь")}</h2>
                <TierBadge tier={match.tier} chance={match.chance} />
                <span className="rounded-pill bg-route-50 px-2.5 py-0.5 text-xs font-bold text-route-700">
                  {t("совпадение")} {match.score}%
                </span>
              </div>
              <p className="mt-1 text-sm text-muted">
                {t(TIER_HINT[match.tier])}
                {t(". Это ориентир по открытой статистике, а не гарантия.")}
              </p>
              <ul className="mt-4 space-y-1.5">
                {match.reasons.slice(0, 4).map((r) => (
                  <li key={r.text} className="flex items-start gap-2 text-sm text-ink-soft">
                    <Check className="mt-0.5 size-4 shrink-0 text-success-500" strokeWidth={3} aria-hidden /> {t(r.text)}
                  </li>
                ))}
                {[...match.blockers, ...match.concerns.filter((c) => (c.weight ?? 0) > 0).map((c) => c.text)].slice(0, 3).map((text) => (
                  <li key={text} className="flex items-start gap-2 text-sm text-warn-700">
                    <AlertCircle className="mt-0.5 size-4 shrink-0" aria-hidden /> {t(text)}
                  </li>
                ))}
              </ul>
              <div className="mt-4 rounded-2xl bg-canvas p-4">
                <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted">{t("Из чего складывается шанс")}</p>
            ...
            <p className="rounded-card border border-dashed border-line-strong bg-surface p-4 text-sm text-ink-soft">
              ...
              {t(", чтобы увидеть свой шанс поступления и стоимость с учётом стипендий.")}
            </p>
          )}
        ...
        <aside className="min-w-0 space-y-4 lg:sticky lg:top-6">
          <FactCard icon={Wallet} title={t("Стоимость в год")}>
            <Row label={t("Обучение")} value={t(u.tuition_usd_per_year === 0 ? "бесплатно" : formatUsd(u.tuition_usd_per_year))} />
            <Row label={t("Жильё и жизнь")} value={`≈${formatUsd(u.living_cost_usd_per_year)}`} />
            {complete && match.costs.expectedAid > 0 && (
              <Row label={t("Ожидаемая помощь")} value={`−${formatUsd(match.costs.expectedAid)}`} tone="text-success-700" />
            )}
            <Row label={t("Итого")} value={formatUsd(complete ? match.costs.net : u.tuition_usd_per_year + u.living_cost_usd_per_year)} strong />
          </FactCard>

          <FactCard icon={GraduationCap} title={t("Требования")}>
            <Row label={t("Средний балл")} value={t(u.min_gpa_4 ? `от ${u.min_gpa_4.toFixed(1)} из 4` : "—")} />
            <Row label="IELTS / TOEFL" value={`${u.min_ielts?.toFixed(1) ?? "—"} / ${u.min_toefl ?? "—"}`} />
            <Row
              label="SAT"
              value={t(
                u.sat_required
                  ? `нужен${u.sat_recommended ? `, ${u.sat_recommended}+` : ""}`
                  : u.sat_recommended
                    ? `желательно ${u.sat_recommended}+`
                    : "не нужен",
              )}
            />
            <Row label={t("Подготовительный год")} value={u.requires_foundation ? t("нужен") : t("не нужен")} />
            {u.acceptance_rate != null && <Row label={t("Поступает")} value={t("≈{0}% заявок", u.acceptance_rate)} />}
            {u.entrance_exams.length > 0 && <Row label={t("Экзамены")} value={t(u.entrance_exams.join(", "))} />}
            {u.foundation_note && <p className="pt-1 text-xs text-muted">{t(u.foundation_note)}</p>}
          </FactCard>

          <FactCard icon={CalendarClock} title={t("Дедлайны на {0} год", startYear)}>
            {deadlines.length ? (
              deadlines.map((d) => <Row key={d.label} label={t(d.label)} value={formatDate(d.date)} />)
            ) : (
              <p className="text-sm text-muted">{t("Уточняйте на сайте вуза.")}</p>
            )}
            {u.intake && <Row label={t("Начало учёбы")} value={t(u.intake)} />}
          </FactCard>

          <FactCard icon={Languages} title={t("Обучение")}>
            <Row label={t("Языки")} value={t(u.instruction_languages.map((l) => LANGUAGE_RU[l] ?? l).join(", ") || "—")} />
            {u.programs.length > 0 && <p className="pt-1 text-xs leading-relaxed text-muted">{t(u.programs.slice(0, 6).join(" · "))}</p>}
          </FactCard>

          {uniScholarships.length > 0 && (
            <FactCard icon={HandCoins} title={t("Стипендии")}>
              <ul className="space-y-2">
                {uniScholarships.map((s) => (
                  <li key={s.id} className="text-sm">
                    ...
                    <span className="block text-xs text-muted">{t(COVERAGE_LABEL[s.coverage])}</span>
                  </li>
                ))}
              </ul>
            </FactCard>
      ...
      {similar.length > 0 && (
        <section>
          <h2 className="font-semibold">{t("Похожие вузы")}</h2>
            ...
            {similar.map((m) => (
              ...
              >
                ...
                <div className="p-3.5">
                  ...
                  <p className="mt-2 truncate font-semibold">{t(m.university.name)}</p>
                  <p className="truncate text-xs text-muted">
                    {t(m.university.city)}, {t(COUNTRY_NAME[m.university.country_code] ?? m.university.country)}
                  </p>
                </div>
              </Link>
            ))}
...
async function FactCard({ icon: Icon, title, children }: { icon: typeof Wallet; title: string; children: React.ReactNode }) {
  const t = await getT();
  return (
...
async function Row({ label, value, strong, tone }: { label: string; value: string; strong?: boolean; tone?: string }) {
  const t = await getT();
  return (
    <div className="flex items-baseline justify-between gap-3 text-sm">
      <span className="text-muted">{t(label)}</span>
      <span className={`text-right ${strong ? "font-bold text-ink" : "font-medium"} ${tone ?? ""}`}>{t(value)}</span>
    </div>
  );
}
