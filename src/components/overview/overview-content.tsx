...
export function OverviewContent({ diagnosis, recommended, draft, fresh }: Props) {
  const { strengths, limitations, goal, readiness, metrics, summary } = diagnosis;

  const readinessLabel = readiness.score >= 75 ? "Сильный профиль" : readiness.score >= 55 ? "Хорошая база" : readiness.score >= 35 ? "Есть над чем поработать" : "Начало пути";
  ...
  return (
    <div className="container-page space-y-6 py-6 sm:py-10">
      {fresh && (
        <Reveal>
          <div className="flex items-start gap-3 rounded-card border border-route-100 bg-route-50 p-4 text-route-700">
            ...
            <p className="text-sm">
              <b>Анкета готова!</b> Вот что мы увидели в твоём профиле. Ниже — сильные стороны, ограничения и цель, а дальше
              подбор вузов.
            </p>
          </div>
        </Reveal>
      ...
      <PageHeader
        eyebrow="Этап 2 · Диагностика"
        title="Твой профиль абитуриента"
        description="Короткое резюме: с чем ты идёшь, что может помешать и куда целишься."
        actions={
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]">
        ...
        <Reveal delay={0.05}>
          <Card className="h-full p-5 sm:p-6">
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-muted">
              <Gauge className="size-4 text-brand-600" aria-hidden /> Готовность к поступлению
            </div>
            <div className="mt-3 flex items-center gap-5">
              <ReadinessRing value={readiness.score} />
              <div>
                <p className="text-lg font-semibold">{readinessLabel}</p>
                <p className="mt-1 text-sm text-muted">Оценка по оценкам, английскому, экзаменам, активностям и плану.</p>
              </div>
            </div>
      ...
      <Stagger className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          { icon: GraduationCap, label: "Средний балл", value: metrics.gpa4 != null ? `≈ ${metrics.gpa4.toFixed(1)} / 4.0` : "—", hint: draft.gpa != null ? `${draft.gpa} из ${draft.gpa_scale}` : "не указан" },
          { icon: Languages, label: "Английский", value: metrics.ielts != null ? `IELTS ≈ ${metrics.ielts.toFixed(1)}` : "—", hint: metrics.ieltsSource },
          { icon: Sparkles, label: "SAT", value: metrics.sat != null ? String(metrics.sat) : "—", hint: metrics.sat != null ? "сдан или цель" : "пока нет" },
          { icon: Clock, label: "До старта учёбы", value: metrics.yearsLeft > 0 ? `${metrics.yearsLeft} ${plural(metrics.yearsLeft, ["год", "года", "лет"])}` : "меньше года", hint: `осень ${draft.start_year ?? ""}` },
        ...
        <Reveal>
          <Card className="h-full p-5 sm:p-6">
            <h2 className="flex items-center gap-2 text-lg font-semibold">
              <CheckCircle2 className="size-5 text-success-500" aria-hidden /> Сильные стороны
            </h2>
            {strengths.length ? (
              <ul className="mt-4 space-y-3">
                {strengths.map((s) => (
                  <li key={s.title} className="rounded-2xl bg-success-50/60 p-3.5">
                    <p className="font-semibold text-success-700">{s.title}</p>
                    <p className="mt-1 text-sm text-ink-soft">{s.text}</p>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="mt-4 text-sm text-muted">Пока нечем похвастаться — это нормально. Маршрут поможет усилить профиль.</p>
            )}
          </Card>
        </Reveal>
        <Reveal delay={0.05}>
          <Card className="h-full p-5 sm:p-6">
            <h2 className="flex items-center gap-2 text-lg font-semibold">
              <AlertTriangle className="size-5 text-warn-500" aria-hidden /> Ограничения и риски
            </h2>
            {limitations.length ? (
              <ul className="mt-4 space-y-3">
                {limitations.map((s) => (
                  <li key={s.title} className="rounded-2xl bg-warn-50/70 p-3.5">
                    <p className="font-semibold text-warn-700">{s.title}</p>
                    <p className="mt-1 text-sm text-ink-soft">{s.text}</p>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="mt-4 text-sm text-muted">Серьёзных ограничений не видно. Отличная стартовая позиция!</p>
            )}
          </Card>
        </Reveal>
      </div>
      ...
      <Reveal>
        <Card className="p-5 sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-muted">Под твой профиль сейчас подходит</p>
              <p className="mt-1 font-display text-2xl font-semibold">
                {summary.recommended} {plural(summary.recommended, ["вуз", "вуза", "вузов"])}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              {(["reach", "target", "safety"] as const).map((t) => (
                <span key={t} className="rounded-xl bg-canvas px-3 py-2 text-sm">
                  <b>{summary[t]}</b> · {TIER_LABEL[t]}
                </span>
              ))}
            </div>
          </div>
      ...
      <NextStage
        label="Этап 3 из 5"
        title="Посмотри, какие вузы подходят и почему"
        description="Шансы на поступление, стоимость с учётом стипендий и понятное объяснение для каждого варианта."
...
function ReadinessRing({ value }: { value: number }) {
  ...
  const c = 2 * Math.PI * r;
  return (
    <div className="relative size-24 shrink-0" role="img" aria-label={`Готовность ${value} из 100`}>
      <svg viewBox="0 0 80 80" className="size-full -rotate-90" aria-hidden>
        ...
        <circle cx="40" cy="40" r={r} fill="none" stroke="url(#readiness-g)" strokeWidth="8" strokeLinecap="round" strokeDasharray={c} strokeDashoffset={c * (1 - value / 100)} />
      </svg>
      <span className="absolute inset-0 grid place-items-center">
        <span className="text-center">
          <span className="block font-display text-2xl font-semibold leading-none">{value}</span>
          <span className="text-[10px] text-muted">из 100</span>
        </span>
      </span>
    </div>
  );
