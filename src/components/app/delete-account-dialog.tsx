    ...
    <AnimatePresence>
      {open && (
        <motion.div className="fixed inset-0 z-[70] flex items-end justify-center sm:items-center sm:p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          >
            <button type="button" onClick={onClose} disabled={pending} className="absolute right-3 top-3 grid size-9 place-items-center rounded-lg text-muted hover:bg-canvas" aria-label="Закрыть">
              <X className="size-5" />
            </button>
            ...
            <p className="mt-2 text-sm text-ink-soft">
              {email ? (
                <>
                  Аккаунт <b className="break-all text-ink">{email}</b> и все его данные будут удалены без возможности восстановления:
                </>
              ) : (
                "Аккаунт и все его данные будут удалены без возможности восстановления:"
              )}
            </p>
            ...
            <form action={action} className="mt-5 space-y-3">
              <label htmlFor="delete-confirm" className="block text-sm font-semibold">
                Чтобы подтвердить, введите «{DELETE_CONFIRMATION}»
              </label>
              <Input
                id="delete-confirm"
                name="confirm"
